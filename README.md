...
* **AI Integration:** Google Gemini API (`gemini-3.5-flash`) via the `@google/generative-ai` SDK. Automatically analyses daily student comments to extract positive highlights, kitchen alerts, and suggestions (falls back to a rule-based Local NLP analyzer if no API Key is set).
